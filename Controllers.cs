...
{
    public class StudentGovernment
    {
        ...
        public virtual Task OnGroupChat(Chat chat, User user, UpdateInfo update) => Task.CompletedTask;
        ...
        public StudentGovernment(string botToken)
        {
        ...
        public async Task CallKeyboard(Chat chat, string text, ReplyKeyboardMarkup replyKeyboardMarkup)
        {
        ...
        private void RunTask(TaskInfo taskInfo, UpdateInfo updateInfo, Chat chat)
        {
            Func<Task> taskStarter = (chat?.Type) switch
            {
                ...
                ChatType.Group or ChatType.Supergroup or ChatType.Sender => () => OnGroupChat(chat, updateInfo.Message?.From, updateInfo),
